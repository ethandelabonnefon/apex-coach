...
- **Phase 7 — Graphique 8h + correction auto + push notifications (avril 2026)** : les 3 améliorations post-intégration LibreLink qui rendent l'app T1D-grade :
  ...
  - **Push notifications iOS** (infra complète) :
    ...
    - **Cron Vercel** (`vercel.json`) : `*/5 * * * *` — check glycémie toutes les 5 min. Si le plan Hobby bloque la granularité, fallback sur cron-job.org pointant vers `?secret=...`
  - **Env vars totales à configurer sur Vercel** : ANTHROPIC_API_KEY, LIBRELINK_EMAIL, LIBRELINK_PASSWORD, LIBRELINK_CLIENT_VERSION, NEXT_PUBLIC_VAPID_PUBLIC_KEY, VAPID_PRIVATE_KEY, VAPID_SUBJECT, CRON_SECRET (+ les KV_* auto-injectées quand on lie un store KV au projet)
