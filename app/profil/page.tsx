    ...
    <div className="flex flex-col gap-1">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          max={max}
          step={step}
          className="w-full bg-bg-hover border border-border-default rounded-lg px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-[var(--success)]/50 focus:ring-1 focus:ring-[var(--success)]/25 transition-colors"
        />
        {unit && <span className="text-xs text-text-tertiary whitespace-nowrap">{unit}</span>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="w-full bg-bg-hover border border-border-default rounded-lg px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-[var(--success)]/50 focus:ring-1 focus:ring-[var(--success)]/25 transition-colors appearance-none"
      >
        {options.map((o) => (
    ...
    <div className="flex flex-col gap-2">
        ...
        {tags.map((tag) => (
          <span key={tag} className="inline-flex items-center gap-1">
            ...
            <button
              ...
              className="text-text-tertiary hover:text-[var(--error)] text-xs transition-colors -ml-1"
            >
              x
            </button>
          </span>
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder="Ajouter..."
          className="flex-1 bg-bg-hover border border-border-default rounded-lg px-3 py-1.5 text-sm text-text-primary placeholder:text-text-disabled focus:outline-none focus:border-[var(--success)]/50 focus:ring-1 focus:ring-[var(--success)]/25 transition-colors"
        />
    ...
    <div className="mt-6">
      <Card glow="blue">
        <div className="flex items-center justify-between mb-4">
          ...
          <a href="/profil/diagnostic" className="text-xs text-[var(--chart-2)] hover:text-[var(--chart-2)]/80 transition-colors">
            Mettre à jour
          </a>
        </div>
        <p className="text-xs text-text-tertiary -mt-2 mb-4">Dernière mesure : {lastDate}</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {Object.entries(MEASUREMENT_LABELS).map(([key, label]) => {
            ...
            return (
              <div key={key} className="p-3 rounded-lg bg-bg-hover">
                <p className="text-[10px] text-text-tertiary uppercase tracking-wider">{label}</p>
                <p className="text-lg font-semibold mt-0.5">
                  {val || "—"}
                  {val && <span className="text-xs text-text-tertiary font-normal ml-1">cm</span>}
                </p>
                {delta && (
                  <p className={`text-[10px] mt-0.5 ${delta.value > 0 ? "text-[var(--success)]" : delta.value < 0 ? "text-[var(--error)]" : "text-text-disabled"}`}>
                    {delta.display}
                  </p>
                )}
              </div>
        ...
        {latest.analysis?.ratios && latest.analysis.ratios.length > 0 && (
          <div className="mb-6 pt-4 border-t border-border-subtle">
            <p className="text-xs text-text-tertiary font-medium mb-3 uppercase tracking-wider">Ratios clés</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {latest.analysis.ratios.slice(0, 6).map((r, i) => (
                <div key={i} className="flex items-center justify-between p-2 rounded-lg bg-bg-hover">
                  <span className="text-[10px] text-text-secondary truncate mr-2">{r.label.replace("Ratio ", "")}</span>
        ...
        {chartData.length > 1 && (
          <div className="pt-4 border-t border-border-subtle">
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs text-text-tertiary font-medium uppercase tracking-wider">Évolution</p>
            </div>
            ...
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData}>
                  ...
                  <Tooltip
                    contentStyle={{ background: "var(--bg-elevated)", border: "1px solid rgba(0,0,0,0.1)", borderRadius: "8px", fontSize: "12px" }}
        ...
        {diagnosticHistory.length > 1 && (
          <div className="pt-4 mt-4 border-t border-border-subtle">
            <p className="text-xs text-text-tertiary font-medium mb-2 uppercase tracking-wider">Historique des mesures</p>
              ...
              {diagnosticHistory.slice(0, 5).map((entry, i) => {
                ...
                return (
                  <div key={entry.id} className="flex items-center gap-2 text-xs">
                    <div className={`w-1.5 h-1.5 rounded-full ${i === 0 ? "bg-[var(--success)]" : "bg-black/20"}`} />
                    <span className="text-text-secondary">{date}</span>
                    <span className="text-text-disabled">— Mesure complète</span>
...
export default function ProfilPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto pb-32">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card glow="green">
          ...
          <div className="mt-6 pt-4 border-t border-border-subtle">
            <p className="text-xs text-text-tertiary mb-3">Morphologie</p>
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="flex justify-between p-2 rounded-lg bg-bg-hover">
              ...
              <div className="flex justify-between p-2 rounded-lg bg-bg-hover">
                <span className="text-text-tertiary">Bras</span>
                <span className="text-text-secondary">{profile.morphology.armLength ?? "Non mesure"}</span>
              </div>
              <div className="flex justify-between p-2 rounded-lg bg-bg-hover">
                <span className="text-text-tertiary">Femur</span>
                <span className="text-text-secondary">{profile.morphology.femurLength ?? "Non mesure"}</span>
              </div>
              <div className="flex justify-between p-2 rounded-lg bg-bg-hover">
                <span className="text-text-tertiary">Torse</span>
                <span className="text-text-secondary">{profile.morphology.torsoLength ?? "Non mesure"}</span>
              </div>
              <div className="flex justify-between p-2 rounded-lg bg-bg-hover">
                <span className="text-text-tertiary">Epaules</span>
                <span className="text-text-secondary">{profile.morphology.shoulderWidth ?? "Non mesure"}</span>
              </div>
              <div className="flex justify-between p-2 rounded-lg bg-bg-hover">
                <span className="text-text-tertiary">Hanches</span>
                <span className="text-text-secondary">{profile.morphology.hipWidth ?? "Non mesure"}</span>
              </div>
            </div>
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-xs text-text-tertiary">Mon programme d&apos;insuline</p>
              <Link
                href="/diabete/parametres"
                className="text-xs text-[var(--accent-2)] hover:text-[var(--accent-2)]/80 transition-colors"
              >
                Modifier →
              </Link>
            </div>
            <div className="space-y-2">
              {profileDiabeteSlots.map((slot) => {
                ...
                return (
                  <div
                    key={slot.key}
                    className="p-3 rounded-lg bg-bg-hover text-sm text-text-primary"
                  >
                    <span className="text-lg font-bold text-[var(--accent-2)] mr-1">{display}</span>
                    <span className="text-[var(--accent-2)]/80">{unitWord} pour 10g de glucide</span>{" "}
          ...
          <div className="space-y-4">
            ...
            <div className="pt-3">
              ...
              <div className="flex justify-between text-[10px] text-text-tertiary mt-1">
                <span className="text-[var(--chart-2)]">P {Math.round((targetProtein * 4 / (targetProtein * 4 + targetCarbs * 4 + targetFat * 9)) * 100)}%</span>
                <span className="text-[var(--warning)]">G {Math.round((targetCarbs * 4 / (targetProtein * 4 + targetCarbs * 4 + targetFat * 9)) * 100)}%</span>
                <span className="text-[var(--accent-2)]">L {Math.round((targetFat * 9 / (targetProtein * 4 + targetCarbs * 4 + targetFat * 9)) * 100)}%</span>
              </div>
            </div>
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 rounded-xl bg-bg-hover space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-text-tertiary">CGM actif</span>
                <span>{profile.hasCGM ? <Badge color="green">Oui</Badge> : <Badge color="red">Non</Badge>}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-text-tertiary">Type diabete</span>
                <span><Badge color="blue">{profile.diabetesType}</Badge></span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-text-tertiary">Insuline rapide</span>
                <span className="text-text-secondary">{insulinRapid}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-text-tertiary">Dose basale</span>
                <span className="text-text-secondary">{basalDose} U/jour</span>
              </div>
            </div>
          </div>
        </Card>
      ...
      {/* Sticky save bar */}
      <div className="fixed bottom-16 lg:bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-bg-primary via-bg-primary/95 to-transparent pointer-events-none z-40">
