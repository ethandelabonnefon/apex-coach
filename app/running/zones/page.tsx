...
const ZONE_CONFIG = [
  ...
  },
  {
    ...
    color: "border-[var(--chart-2)]/30 bg-[var(--chart-2)]/5",
    textColor: "text-[var(--chart-2)]",
    accentBg: "bg-[var(--chart-2)]",
      ...
      <Card glow="blue" className="mb-8">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-4xl font-bold text-[var(--chart-2)]">
              {vma.toFixed(1)} <span className="text-lg text-text-tertiary font-normal">km/h</span>
            </p>
          ...
          <div className="flex gap-3">
            <div className="text-center">
              <p className="text-xs text-text-tertiary mb-1">Allure VMA</p>
              <p className="text-lg font-bold text-text-primary">
                {formatPace(60 / vma)} <span className="text-xs text-text-tertiary">/km</span>
              </p>
            </div>
            <div className="text-center">
              <p className="text-xs text-text-tertiary mb-1">Phase actuelle</p>
            ...
            <Card key={cfg.key} className={`border ${cfg.color}`}>
              <div className="flex flex-col md:flex-row gap-4">
                ...
                <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    <p className="text-xs text-text-tertiary mb-0.5">Vitesse</p>
                    <p className="text-sm font-semibold text-text-primary">
                      {zone.speedKmh.min.toFixed(1)} - {zone.speedKmh.max.toFixed(1)}
                      <span className="text-text-tertiary text-xs ml-1">km/h</span>
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-text-tertiary mb-0.5">Allure</p>
                    <p className="text-sm font-semibold text-text-primary">
                      {formatPace(zone.paceMinKm.min)} - {formatPace(zone.paceMinKm.max)}
                      <span className="text-text-tertiary text-xs ml-1">/km</span>
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-text-tertiary mb-0.5">FC</p>
                    <p className="text-sm font-semibold text-text-primary">
                      {zone.hrPercent.min} - {zone.hrPercent.max}
                      <span className="text-text-tertiary text-xs ml-1">% FCmax</span>
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-text-tertiary mb-0.5">Sensation</p>
                    <p className="text-xs text-text-secondary">{zone.feeling}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-3 pt-3 border-t border-border-subtle">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-sm text-text-secondary">
                    <span className="text-text-secondary font-medium">Objectif :</span> {zone.purpose}
                  </p>
                </div>
              </div>
            </Card>
        ...
        {[
          ...
          <Card key={p.phase}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-text-tertiary">{p.weeks}</span>
            </div>
        ...
        {races.map((race) => {
          ...
          return (
            <Card key={race.label} glow="blue">
              <p className="text-xs text-text-tertiary uppercase tracking-wider mb-1">{race.label}</p>
              <p className="text-2xl font-bold text-[var(--chart-2)] mb-2">
                {formatTime(pred.predictedTimeMinutes)}
              </p>
              <div className="space-y-1.5">
                <div className="flex justify-between">
                  <span className="text-xs text-text-tertiary">Allure</span>
                  <span className="text-xs text-text-secondary font-medium">
                    {formatPace(pred.predictedPace)} /km
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-xs text-text-tertiary">Vitesse</span>
                  <span className="text-xs text-text-secondary font-medium">
                    {pred.predictedSpeed.toFixed(1)} km/h
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-xs text-text-tertiary">% VMA</span>
                  <span className="text-xs text-text-secondary font-medium">
                    {((pred.predictedSpeed / vma) * 100).toFixed(0)}%
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-xs text-text-tertiary">Precision</span>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {ZONE_CONFIG.map((cfg) => {
                const zone = zones[cfg.key];
                return (
                  <tr key={cfg.key} className="border-b border-border-subtle last:border-0">
                    ...
                    <td className="py-3 text-text-secondary text-xs">{zone.purpose}</td>
                  </tr>
                );
              })}
            </tbody>
