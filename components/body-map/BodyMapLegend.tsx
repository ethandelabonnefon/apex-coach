        ...
        return (
          <div key={status}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] text-text-disabled">— {STATUS_ICONS[status]}</span>
            </div>
      ...
      {muscles.filter((m) => m.status === "unknown").length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs text-text-tertiary">Non évalués</span>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {muscles
              .filter((m) => m.status === "unknown")
              .map((muscle) => (
                <span key={muscle.id} className="px-2.5 py-1 rounded-lg text-xs text-text-disabled bg-bg-hover border border-border-subtle">
                  {muscle.name}
                </span>
              ))}
          </div>
