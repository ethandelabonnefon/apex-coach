    ...
    <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
      <div className="w-full max-w-lg max-h-[80vh] overflow-y-auto">
        <Card>
          ...
          <p className="text-xs text-black/40 mb-4">Modifie les valeurs que tu souhaites mettre à jour.</p>

          <div className="space-y-3">
            {config.fields.map((field) => (
              <div key={field.key}>
                <label className="block text-xs text-black/50 mb-1.5">{field.label}</label>
                {field.type === "select" ? (
                  <select
                    value={data[field.key] || ""}
                    onChange={(e) => update(field.key, e.target.value)}
                    className="w-full bg-black/[0.04] border border-black/[0.08] rounded-xl px-4 py-2.5 text-sm text-text-primary focus:outline-none focus:border-[#34c759]/50 focus:ring-1 focus:ring-[#34c759]/20 transition-colors appearance-none"
                  >
                  ...
                  <textarea
                    ...
                    onChange={(e) => update(field.key, e.target.value)}
                    rows={2}
                    className="w-full bg-black/[0.04] border border-black/[0.08] rounded-xl px-4 py-2.5 text-sm text-text-primary focus:outline-none focus:border-[#34c759]/50 focus:ring-1 focus:ring-[#34c759]/20 transition-colors resize-none"
                  />
                ) : (
                  <div className="relative">
                    <input
                      ...
                      value={data[field.key] || ""}
                      onChange={(e) => update(field.key, e.target.value)}
                      className="w-full bg-black/[0.04] border border-black/[0.08] rounded-xl px-4 py-2.5 text-sm text-text-primary placeholder:text-black/20 focus:outline-none focus:border-[#34c759]/50 focus:ring-1 focus:ring-[#34c759]/20 transition-colors"
                    />
                    {field.unit && (
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-black/30">{field.unit}</span>
                    )}
                  </div>
                )}
              </div>
          ...
          <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-black/[0.06]">
