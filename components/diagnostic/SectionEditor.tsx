    ...
    <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
      <div className="w-full max-w-lg max-h-[80vh] overflow-y-auto">
        <Card>
          ...
          <p className="text-xs text-text-tertiary mb-4">Modifie les valeurs que tu souhaites mettre à jour.</p>
              ...
              <div key={field.key}>
                ...
                {field.type === "select" ? (
                  <select
                    ...
                    className="w-full bg-bg-hover border border-border-default rounded-xl px-4 py-2.5 text-sm text-text-primary focus:outline-none focus:border-[var(--success)]/50 focus:ring-1 focus:ring-[var(--success)]/20 transition-colors appearance-none"
                  >
                  ...
                  <textarea
                    ...
                    onChange={(e) => update(field.key, e.target.value)}
                    rows={2}
                    className="w-full bg-bg-hover border border-border-default rounded-xl px-4 py-2.5 text-sm text-text-primary focus:outline-none focus:border-[var(--success)]/50 focus:ring-1 focus:ring-[var(--success)]/20 transition-colors resize-none"
                  />
                ) : (
                  <div className="relative">
                    <input
                      ...
                      value={data[field.key] || ""}
                      onChange={(e) => update(field.key, e.target.value)}
                      className="w-full bg-bg-hover border border-border-default rounded-xl px-4 py-2.5 text-sm text-text-primary placeholder:text-text-disabled focus:outline-none focus:border-[var(--success)]/50 focus:ring-1 focus:ring-[var(--success)]/20 transition-colors"
                    />
                    {field.unit && (
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-text-tertiary">{field.unit}</span>
                    )}
                  </div>
                )}
              </div>
